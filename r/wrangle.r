...
lama <- raw %>% 
  ...
  select(-matches("^abandoned|^accepted|^address|^apply|^asbestos|^bounding"),
         ...
         -matches("^tax bill|^tier|^tool|^trash"),
         -matches("^violation codes|^visible|^writ|^zoning"),
         -matches("score"))
...
structures <- lama %>% 
  filter(is_this_a_vacant_lot == "NO",
         occupancy_use %in% c("Commercial",
                              "Industrial",
                              "Mixed Use",
                              "Residential"),
         buildings_occupied == "No",
         is_the_property_occupied == "NO") %>% 
  select(assessor_id, tax_bill = pin, 4:6, everything(),
         -c(id,
            buildings_occupied,
            decision_date,
            in_national_historic_district,
            in_neighborhood_conservation_district,
            is_the_property_occupied,
            is_this_a_vacant_lot,
            mva_value))
...
write_feather(decisions, "./data/decisions.feather")
